...
def load_index(lda_model, corpus, index_path=paths.index_path):
    ...
    #     index = similarities.MatrixSimilarity.load(index_path)
        ...
        load_index.index = similarities.MatrixSimilarity(
            lda_model[corpus]
...
class MKR:
    def __init__(self, args, n_user, n_items, n_entities, n_relations):
    ...
    def _build_model(self):
        print("Build models")
        self.MKR_model = MKR_model(
            self.args, self.n_user, self.n_item, self.n_entity, self.n_relation
        )
        # if torch.cuda.device_count() >= 1:
            # self.MKR_model = torch.nn.DataParallel(self.MKR_model)
        if _CUDA:
            self.MKR_model = self.MKR_model.cuda()
    ...
    def _build_ops(self):
        self.optimizer_rs = torch.optim.Adam(
            self.MKR_model.fine_tune_parameters(), lr=self.args.lr_rs
        )
        
        self.optimizer_kge = torch.optim.Adam(
            self.MKR_model.fine_tune_parameters(), lr=self.args.lr_kge
        )

    def _inference_rs(self, inputs, inference=False):
        # Inputs
        self.user_indices = inputs[:, 0].long().to(self.device, non_blocking=True)
        self.item_indices = inputs[:, 1].long().to(self.device, non_blocking=True)
        ...
        self.head_indices = (
            inputs[:, 1].long().to(self.device, non_blocking=True)
        )  # this means that the head indices is from the same data as items
        ...
        if inference:
            ...
            # assert isinstance(self.user_indices,Iterable),self.user_indices
            torch.autograd.set_grad_enabled(False)
            for user_index, item_index, head_index in zip(
                self.user_indices, self.item_indices, self.head_indices
            ):
                *_, score = self.MKR_model(
                    user_indices=user_index.view(-1),
                    item_indices=item_index.view(-1),
    ...
    def _inference_kge(self, inputs):
        ...
        if not self.user_enhanced:
            self.item_indices = inputs[:, 0].long().to(self.device, non_blocking=True)
        else:  # head and user/item are same indices
            self.user_indices = inputs[:, 0].long().to(self.device, non_blocking=True)

        self.head_indices = inputs[:, 0].long().to(self.device, non_blocking=True)
        self.relation_indices = inputs[:, 1].long().to(self.device, non_blocking=True)
        self.tail_indices = inputs[:, 2].long().to(self.device, non_blocking=True)
    ...
    def loss_rs(self, user_embeddings, item_embeddings, scores, labels):
        ...
        for name, param in self.MKR_model.named_parameters():
            if (
                param.requires_grad
                and ("embeddings_lookup" not in name)
    ...
    def train_rs(self, inputs, show_grad=False, glob_step=None):
        ...
        loss_rs, base_loss_rs, l2_loss_rs = self.loss_rs(
            user_embeddings, item_embeddings, scores, labels
        )
    ...
    def train_kge(self, inputs, show_grad=False, glob_step=None):
        ...
        loss_kge, base_loss_kge, l2_loss_kge = self.loss_kge(
            scores_kge, head_embeddings, tail_embeddings
        )
    ...
    def _attatch_train_sparse(self, train_sparse):
        if self.user_enhanced == 1 or self.user_enhanced == 0:  # user enhanced
            ...
            train_texts_bow = self.id2text.loc[
                range(self.n_item, self.n_item + self.n_user), "description"
            ].apply(self.lda_dict.doc2bow)
            self.index = load_index(self.lda, train_texts_bow)
        elif self.user_enhanced == -10:  # item enhanced:
            # 获取所有item的des
            train_texts_bow = self.id2text.loc[range(self.n_item), "description"].apply(
                self.lda_dict.doc2bow
            )
            self.index = load_index(self.lda, train_texts_bow)

    def topk_eval(
        self, user_list, train_record, test_record, item_set, k_list, train_sparse=None
    ):
        ...
        for user in tqdm(user_list):
            ...
            scores = self._get_scores(
                np.array([user] * len(test_item_list)),
    ...
    def _get_scores(self, user, item_list, head_list):
        ...
        for user_index, item_index, head_index in zip(
            ...
            *_, score = self.MKR_model(
                user_indices=user_index.view(-1),
                item_indices=item_index.view(-1),
                head_indices=head_index.view(-1),
                relation_indices=None,
                tail_indices=None,
            )
            scores.append(score)
        ...
        #     if _CUDA:
        #         sim_vec = sim_vec.cuda()
        #         self.train_sparse=self.train_sparse.cuda()
        ...
        # scores = 0.0*lda_scores + 1.0*scores
        return scores
    ...
    def get_NDCG_MAP(self, item_sorted, record):
        ...
        hit_index = (
            []
        )  # record the index those hit the gound truth, for the map calculation
        predict_rels = []  # record the sequence of rel for the calculation of NDCG
        for i, item in enumerate(item_sorted):
            if (
                item in record
            ):  # if in record then rel is 1, means that item has an interact with the user
        ...
        _index_of_predicts = np.array(
            [i for i in range(1, len(item_sorted) + 1)]
        )  # generate the index of the predict sequence,start from 1
        # NDCG
        IDCG = np.sum(
            record_array[: len(_index_of_predicts)] / np.log((_index_of_predicts + 1))
        )  # remember that np.log is log_e
        DCG = np.sum(
            (np.exp2(np.array(predict_rels)) - 1) / np.log(_index_of_predicts + 1)
        )  # dcg = (2**rel -1)/log(i+1) //here i satrt from 1 our i also from 1
        # MAP
        # hit index from 0
        _map_hit_index = (
            np.array(hit_index) + 1
        )  # idx start from 0 but in map calculation the denominator start from 1 so add 1
        MAP = np.sum(_index_of_predicts[: len(_map_hit_index)] / _map_hit_index) / len(
            record
        )
        return DCG / IDCG, MAP
